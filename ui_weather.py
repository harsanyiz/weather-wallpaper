...
def draw_weather_widget(img, weather, icon_img, feel_icon_img, rainq_icon_img, wind1_icon_img, para_icon_img, forecast_icons, sunrise_icon_img, sunset_icon_img, namedays, tz_offset):
    ...
    # Automatikus sötét/világos mód a háttér alapján
    ...
    main_content_w = ICON_DISPLAY_SIZE + 40 + t_w + 60 + 110 # Ikon + hézag + Temp + hézag + Info_oszlop
    ...
    draw.text((tx + 20 + day_w + 30, mid_y - 65), weather["weather_hu"].upper(), font=f_h, fill=colors["dim"])
    ...
    # Eső (Fent)
    if rainq_icon_img: paste_icon(img, rainq_icon_img, info_x, mid_y - 42, size=42) # Megnövelt ikon
    ...
    # 5. NÉVNAP & IDŐ (IDŐ VISSZATÉVE A SAROKBA)
    ...
    # Frissítési idő a jobb felső sarokba (visszahelyezve)
