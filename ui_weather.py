...
# ============================================================
# 4K-S HORIZONTÁLIS DESIGN - STEVE JOBS KIADÁS
# ============================================================
WIDGET_WIDTH  = 2400
WIDGET_HEIGHT = 200
WIDGET_Y      = 100
...
FONT_TEMP     = 95   # Picit nagyobb a hangsúly kedvéért
FONT_HEADER   = 24
FONT_LABEL    = 28
FONT_VALUE    = 36
FONT_SMALL    = 32   # Jobban olvasható érzet hőfok
FONT_DATETIME = 24
FONT_NAME     = 28
...
WIND_ICON_SIZE     = 36
PARA_ICON_SIZE     = 36
FORECAST_ICON_SIZE = 55   # Megnövelve a jobb láthatóságért
SUN_ICON_SIZE      = 40
ICON_GAP           = 12
...
def draw_weather_widget(img, weather, icon_img, feel_icon_img,
    ...
    # ── SZEKCIÓ 1: AKTUÁLIS (Baseline igazítva) ─────────────────────────────
    SEC1_W = 580
    ...
    t_w, t_h = draw.textbbox((0, 0), temp_txt, font=f_t)[2:4]
    f_w = draw.textbbox((0, 0), feel_txt, font=f_s)[2]
    
    main_w = ICON_DISPLAY_SIZE + 40 + t_w + 30 + FEEL_ICON_SIZE + 10 + f_w
    ...
    # Fejléc
    draw.text((tx, mid_y - 75), f"{day_txt}  {desc_txt}", font=f_h, fill=colors["dim"])
    # Nagy hőfok
    draw.text((tx, mid_y - 20), temp_txt, font=f_t, fill=colors["main"])
    
    # Érzet igazítás az alapvonalhoz (temp_y + t_h környéke)
    feel_x = tx + t_w + 30
    baseline_y = mid_y + 45
    if feel_icon_img:
        paste_icon(img, feel_icon_img, feel_x, baseline_y - FEEL_ICON_SIZE - 5, size=FEEL_ICON_SIZE)
    draw.text((feel_x + FEEL_ICON_SIZE + 10, baseline_y - 35), feel_txt, font=f_s, fill=colors["dim"])

    curr_x += SEC1_W
    draw.line([(curr_x, by + 50), (curr_x, by + bh - 50)], fill=colors["line"], width=2)

    # ── SZEKCIÓ 2 & 3: ADATOK (Középre zárt oszlopok) ────────────────────────
    for data_w, items in [(280, [(weather['wind_kmh'], "km/h", wind_icon_img), (weather['humidity'], "%", para_icon_img)]),
        ...
        for i, (val, unit, icon) in enumerate(items):
            ...
            item_x = sec_mid - ((WIND_ICON_SIZE + 15 + v_w) // 2)
            y_pos = mid_y - 60 if i == 0 else mid_y + 10
            if icon: paste_icon(img, icon, item_x, y_pos + 5, size=WIND_ICON_SIZE)
            draw.text((item_x + WIND_ICON_SIZE + 15, y_pos), txt, font=f_v, fill=colors["main"])
        curr_x += data_w
        draw.line([(curr_x, by + 50), (curr_x, by + bh - 50)], fill=colors["line"], width=2)

    # ── SZEKCIÓ 4: FORECAST (Optikai egyensúly) ─────────────────────────────
    SEC4_W = 500
    slot_w = SEC4_W // 3
    for i, day_entry in enumerate(weather["forecast"]):
        ...
        draw.text((slot_mid - vw // 2, mid_y + 10), f_val, font=f_fv, fill=colors["main"])
    ...
    # ── DÁTUM + IDŐ (Margó fixálva) ──────────────────────────────────────────
    datetime_txt = weather["now_dt"].strftime("%Y.%m.%d  %H:%M")
    dt_w = draw.textbbox((0, 0), datetime_txt, font=f_dt)[2]
    draw.text((bx + bw - dt_w - INNER_MARGIN, by + 20), datetime_txt, font=f_dt, fill=(160, 160, 160, 200))
