...
def draw_weather_widget(img, weather, icon_img, feel_icon_img, rainq_icon_img, wind1_icon_img, para_icon_img, forecast_icons, sunrise_icon_img, sunset_icon_img, namedays, tz_offset):
    ...
    # Automatikus sötét/világos mód a háttér alapján - XIAOMI színek
    brightness = ImageStat.Stat(img.crop((bx, by, bx + bw, by + bh)).convert("L")).mean[0]
    
    if brightness > 145:
        # Világos mód - Xiaomi fehér kártya
        colors = {"main": (30, 30, 45, 255), "dim": (100, 100, 120, 200), "line": (0, 0, 0, 15), "accent": (0, 150, 210, 255)}
    else:
        # Sötét mód - Xiaomi sötét kártya
        colors = {"main": (255, 255, 255, 255), "dim": (180, 180, 200, 180), "line": (255, 255, 255, 20), "accent": (0, 200, 255, 255)}
    ...
    # Fejléc (NAP + ÁLLAPOT) - ÁLLAPOT akcentussal
    draw.text((tx + 20, mid_y - 65), get_day_hu(weather["now_dt"]).upper(), font=f_h, fill=colors["dim"])
    ...
    if rainq_icon_img: paste_icon(img, rainq_icon_img, info_x, mid_y - 42, size=42)
    draw.text((info_x + 52, mid_y - 42), rain_txt, font=f_s, fill=colors["dim"])
    
    if feel_icon_img: paste_icon(img, feel_icon_img, info_x + 4, mid_y + 8, size=36)
    ...
    # 2. SZÉL & PÁRA (eredeti, változatlan)
    ...
    # 3. NAPKELTE / NAPNYUGTA (eredeti, változatlan)
    ...
    # 4. ELŐREJELZÉS (3 NAP) (eredeti, változatlan)
    ...
    for i, day_entry in enumerate(weather["forecast"]):
        ...
        if i < len(forecast_icons): paste_icon(img, forecast_icons[i], sm - 25, mid_y - 75, size=50)
    ...
    # 5. NÉVNAP & IDŐ (eredeti, változatlan)
