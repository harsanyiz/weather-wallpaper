...
# ============================================================
# 4K KONFIGURÁCIÓ - MINDEN IKONNAL (FŐ, RÉSZLETEK, ELŐREJELZÉS)
# ============================================================
CITY = "Budapest"
WIDGET_Y = 100        
OFFSET_LEFT = 135     
...
def main():
    ...
    # --- 1. SZEKCIÓ: FŐ IKON (JAVÍTOTT KISEBB) + HŐFOK ---
    icon_path = f"images/PNG/{icon_file}.png"
    if os.path.exists(icon_path):
        icon_img = Image.open(icon_path).convert("RGBA").resize((120, 120), Image.Resampling.LANCZOS)
        img.paste(icon_img, (int(curr_x), int(mid_y - 80)), icon_img)
        
        # DERÜLT felirat az ikon alá
    ...
    # --- 2. SZEKCIÓ: RÉSZLETEK IKONOKKAL (ÉRZET, SZÉL, PÁRA) ---
    details = [
        ("Érzet", f"{round(curr_r['main']['feels_like'])}°C", "day_clear.png"), # Ideiglenes hőmérő helyett
        ("Szél", f"{round(curr_r['wind']['speed']*3.6)} km/h", "wind.png"),
        ("Pára", f"{curr_r['main']['humidity']}%", "rain.png")
    ]
    
    for label, val, i_file in details:
        # Kis ikon a felirat mellé
        ...
        if os.path.exists(det_icon_path):
            ...
            img.paste(d_icon, (int(curr_x - 45), int(mid_y - 45)), d_icon)
        ...
        curr_x += max(draw.textbbox((0,0), label.upper(), font=f_l)[2], draw.textbbox((0,0), val, font=f_v)[2]) + 110
