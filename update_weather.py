...
def main():
    try:
        ...
        if nameday_text:
            # Minden elválasztót egységesítünk, majd üres elemeket kiszűrünk
        ...
        feels_x = curr_x
        icon_gap = 32 if feels_icon else 0
        if feels_icon:
            img.paste(feels_icon, (feels_x, mid_y + 6), feels_icon)
        draw.text((feels_x + icon_gap, mid_y + 5), f"{weather_hu}  {feels_label}", font=f_d, fill=c_dim)
        ...
        # Nap szekció fix szélesség: elég a tartalomnak + légzés
        sun_section_w = 320
        ...
        # available_width = sun_section_w (ugyanabba a 320px-es sávba kerül, alul)
        WIND_ICON_SIZE = 26
        ...
        max_nd_w = widget_right - nameday_x - 20  # max szélesség
        
        if nameday_one_line:
            upd_text = f"Frissítve: {local_now.strftime('%H:%M')}"
            
            # Auto-shrink ha a névsor túl széles
            f_nd = f_n
