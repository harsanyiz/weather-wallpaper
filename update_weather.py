...
def get_icon_name(weather_id, is_night):
    """Valós időjárás ID alapján ikon választás a TE ICONS_PNG80 mappádhoz"""
    
    # Zivatar (thunder)
    if weather_id in range(200, 233):
        return f"{'night' if is_night else 'day'}_rain_thunder"
    
    # Ónos eső / sleet
    if weather_id in [511, 611, 612, 613, 615, 616]:
        return f"{'night' if is_night else 'day'}_sleet"
    
    # Eső / rain
    if weather_id in range(500, 532):
        return f"{'night' if is_night else 'day'}_rain"
    
    # Hó / snow
    if weather_id in range(600, 623):
        return f"{'night' if is_night else 'day'}_snow"
    ...
    # Derült
    if weather_id == 800:
        return f"{'night' if is_night else 'day'}_clear"
    
    # Pár felhő
    if weather_id == 801:
        return f"{'night' if is_night else 'day'}_partial_cloud"
...
def load_icon(name):
    """PNG ikon betöltése a TE ICONS_PNG80 mappából"""
...
def paste_icon(img, icon, cx, cy):
    """Ikon beillesztése középre igazítva"""
...
def main():
    ...
    # --- 1. SZEKCIO: NAP + HOMERSEKLET + LEIRAS + IKON ---
